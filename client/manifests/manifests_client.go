...
func (a *Client) SaveManifest(params *SaveManifestParams, opts ...ClientOption) (*SaveManifestAccepted, error) {
	...
	op := &runtime.ClientOperation{
		...
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &SaveManifestReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*SaveManifestAccepted)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for saveManifest: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
